...
const authRepository = require('../repositories/postgressql.repository');
...
const AuthController = {
  ...
  signup: async (req, res) => {
    ...
    try {
      ...
      let { email, user_name, dob, address, password, mobile, } = req.body;
      ...
      if (!mobile==true) {
        ...
        // Check if the user_name Exists as Same UserName cannot be created
         ...
         let bodyData=[email,user_name,dob,address,convertedPassword,mobile];
  ...
  usersList: async (req, res) => {
    try {
      ...
      //Code to Get emailVerificationCode From Db with other fields to copy to userTable
       let response = await authRepository.usersList(code,'mobileOtpVerification');
       let message = response.headers.message;
       console.log(message);
       responseUtils.returnStatusCodeWithMessage(res, 200, message);
