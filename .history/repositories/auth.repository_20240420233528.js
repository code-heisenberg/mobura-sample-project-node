...
class AuthRepository {
    registerUser =  async (body,emailverificationcode,emailStatus) => {
        ...
        if(emailStatus=="sentForVerificationCode")
        {
          ...
          let { otp: extractedOtp, expiry: extractedExpiry } = otp_Time;
            console.log(otp_Time.expiry);
            ...
            if (otp_Time.otp === otp) {
                // Success: OTP matched
                otp_Time.delete; // Remove user data after successful verification
          ...
          let result = await UserModel.tempCreateUser(tempuserdata.user_id,tempuserdata.email,tempuserdata.userName,tempuserdata.dob,tempuserdata.address,tempuserdata.password,tempuserdata.mobile,emailverificationcode,extractedOtp,extractedExpiry);
        }
