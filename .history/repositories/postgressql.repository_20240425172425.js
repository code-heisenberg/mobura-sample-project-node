 ...
 catch(Error)
{
 ...
 usersList = async () => {
    ...
    getAllUserList()
  .then(users => {
    // Assuming structureResponse is a function that formats the response
    const response = structureResponse({ 'user_name': '', 'token': '' }, 1, 'Current Users-List => ' + users.map(user => user.user_name).join(', '));
