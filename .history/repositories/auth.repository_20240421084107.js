...
class AuthRepository {
    registerUser =  async (body,emailverificationcode,emailCodeStatus) => {
        ...
        {
            console.log(emailCodeStatus);
        let getOtp = await UserModel.findByEmailOtpCode(otp_Time.otp);
        console.log(getOtp);
        if ( emailverificationcode == getOtp) {
            // Success: OTP matched
            // Remove user data after successful verification
            //res.send('Email-OTP verified successfully');
            let userdetails = UserModel.findByEmailOtpCodeAllData(otp_Time.otp);
