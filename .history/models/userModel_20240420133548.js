...
const UserModel = {
  ...
  userLogin: async ({name,token,activity,date}) => {
    let activity="active";
    let date = Date();
    const sql = `INSERT INTO ${tables.login} (name,token,activity,date) VALUES (?,?,?,?)`;
    //const values = ;
    try {
     const result = await db.DBService.query(sql, [name,token,activity,date]);
    } catch (error) {
      throw new Error('Database error: ' + error.message);
    }    
