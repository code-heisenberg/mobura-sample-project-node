...
const UserModel = {
  ...
  tempCreateUser: async (user_id,email,userName,dob,address,password,mobile,emailVerificationCode) => {
    const hashedPassword = await bcrypt.hash(password, 10);
    const sql = 'INSERT INTO usertemp (user_id,email,userName,dob,address,password,mobile,emailVerificationCode) VALUES (?,?,?,?,?,?,?,?)';
  ...
  findByEmailOtpCode: async (Code) => {
    const sql = 'SELECT emailotp FROM usertemp WHERE emailverificationcode = ?';
    const values = [Code];
    try {
      const [user] = await db.query(sql, values);
      return user;
