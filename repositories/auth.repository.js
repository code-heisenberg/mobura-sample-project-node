...
class AuthRepository {
    registerUser = async (body, CodeStatus, monbileotp) => {
        // Email-OTP Verifications Done Below
        try {
            let otp_Time = [];
            if (CodeStatus == "sentForEmailOtpVerification") {
            ...
            // Verify OTP
            if (CodeStatus == "emailOtpVerification") {
            ...
            if (CodeStatus == "sentForEmailVerification") {
            ...
            if (CodeStatus == "emailVerification") {
            ...
            //let mobileOtp="";
            if (CodeStatus == "sentForMobileOtpVerification") {
                let mobileOtp = mobileOtpVerifications.generateOTP(monbileotp);
                let result = await UserModel.tempCreateUser(body.user_id, body.email, body.userName, body.dob, body.address, body.password, body.mobile, undefined, undefined, undefined, mobileOtp);
                return structureResponse({ 'userName': "", 'With-OTP': "" }, 1, 'Mobile-Otp Verification Pending or In Process');
            }
            if (CodeStatus == "mobileOtpVerification") {
                let userdetails = await UserModel.findByMobileOtpCode(mobileOtp);
                let otpCode = jwt.verify(mobileotp, 'SECRET_KEY');
                if (otpCode == userdetails.mobileotp) {
                    let detailsUser = [userdetails.user_id, userdetails.email, userdetails.userName, userdetails.dob, userdetails.address, userdetails.password, userdetails.mobile];
                    const result = await UserModel.createUser(detailsUser[0], detailsUser[1], detailsUser[2], detailsUser[3], detailsUser[4], detailsUser[5], detailsUser[6]);
                    return structureResponse({ 'userName': userdetails.userName, 'With-OTP': mobileOtp }, 1, 'Mobile-OTP [Verified]=> You Are Now Part Of The System');
                }
            }
        }
        catch (err) {
